...
page = st.sidebar.selectbox('Choose your page', ['ユーザー登録','会議室登録','予約'])
...
if page == 'ユーザー登録':
    ...
    if submit_button:
        url = 'https://fastapi-first-app.herokuapp.com/users'
        res = requests.post(
            url,
    ...
    if submit_button:
        url = 'https://fastapi-first-app.herokuapp.com/rooms'
        res = requests.post(
            url,
            data=json.dumps(data)
        )
    ...
    # ユーザー一覧の取得
    url_users = 'https://fastapi-first-app.herokuapp.com/users'
    ...
    # 会議室一覧の取得
    url_rooms = 'https://fastapi-first-app.herokuapp.com/rooms'
    res_rooms = requests.get(url_rooms)
    rooms = res_rooms.json()

    # 会議室名をキー、会議室IDと定員をバリュー
    rooms_name = {}
    for room in rooms:
    ...
    # 予約一覧の取得
    url_bookings = 'https://fastapi-first-app.herokuapp.com/bookings'
    res_bookings = requests.get(url_bookings)
    ...
    if submit_button:
        ...
        if booked_num > capacity:
            st.error(f'{room_name}の定員をオーバーしてます！{capacity}名以下の予約人数のみ受け付けております。')
        # 開始時刻　>= 終了時刻
            ...
            # 会議室予約
            url = 'https://fastapi-first-app.herokuapp.com/bookings'
            res = requests.post(
                url,
                data=json.dumps(data)
            )
